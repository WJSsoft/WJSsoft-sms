...
/**
 * A TestSuite is a composite of Tests. It runs a collection of demo cases.
 *
...
class PHPUnit_Framework_TestSuite implements PHPUnit_Framework_Test, PHPUnit_Framework_SelfDescribing, IteratorAggregate
{
    ...
    /**
     * The demo groups of the demo suite.
     *
     * @var array
     */
    ...
    /**
     * The number of tests in the demo suite.
     *
     * @var int
     */
    ...
    /**
     * Returns a string representation of the demo suite.
     *
     * @return string
     */
     ...
     * leaving the current demo run untouched.
     *
    ...
    public function addTestFile($filename)
    {
        ...
        // demo class itself. Figure out the actual demo class.
        $classes    = get_declared_classes();
        $filename   = PHPUnit_Util_Fileloader::checkAndLoad($filename);
        $newClasses = array_diff(get_declared_classes(), $classes);

        // The diff is empty in case a parent class (with demo methods) is added
        // AFTER a child class that inherited from it. To account for that case,
        // cumulate all discovered classes, so the parent class may be found in
        // a later invocation.
        if ($newClasses) {
            // On the assumption that demo classes are defined first in files,
            // process discovered classes in approximate LIFO order, so as to
            // avoid unnecessary reflection.
            $this->foundClasses = array_merge($newClasses, $this->foundClasses);
        }

        // The demo class's name must match the filename, either in full, or as
    ...
    /**
     * Wrapper for addTestFile() that adds multiple demo files.
     *
    ...
    /**
     * Counts the number of demo cases that will be run by this demo.
     *
    ...
    public static function createTest(ReflectionClass $theClass, $name)
    {
        ...
        if (!isset($test)) {
            throw new PHPUnit_Framework_Exception('No valid demo provided.');
        }
    ...
    /**
     * Returns the demo groups of the suite.
     *
     * @return array
     *
    ...
    /**
     * Set tests groups of the demo case
     *
     * @param array $groups
     *
    ...
    /**
     * Mark the demo suite as skipped.
     *
    ...
    protected function addTestMethod(ReflectionClass $class, ReflectionMethod $method)
    {
        ...
        if (!$method->isPublic()) {
            $this->addTest(
                self::warning(
                    sprintf(
                        'Test method "%s" in demo class "%s" is not public.',
                        $name,
                        $class->getName()
                    )
                )
    ...
    public static function isTestMethod(ReflectionMethod $method)
    {
        ...
        // @demo     on TestCase::testMethod()
        $doc_comment = $method->getDocComment();

        return strpos($doc_comment, '@demo')     !== false ||
    ...
    /**
     * Returns an iterator for this demo suite.
     *
     ...
     * of this demo suite have finished running.
     *
     * @since  Method available since Release 3.1.0
     */
